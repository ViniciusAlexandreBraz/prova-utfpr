...
PROJECT_NAME=prova-utfpr
STACK_NAME=prova-utfpr-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://prova-utfpr.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t collective/prova-utfpr-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t collective/prova-utfpr-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name prova-utfpr-backend-acceptance -d collective/prova-utfpr-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name prova-utfpr-frontend-acceptance --link prova-utfpr-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/prova-utfpr-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop prova-utfpr-frontend-acceptance
	@docker stop prova-utfpr-backend-acceptance
